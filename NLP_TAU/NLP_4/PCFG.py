...
class PCFG(object):
    ...
    def gentree(self, symbol):
        ...
        rhs = self.random_expansion(symbol)
        if (len(rhs) == 1 and self.is_preterminal(rhs)) or  (''.join(rhs).islower())  :
            if len(rhs) == 1 :
                return "("+str(symbol)+" "+rhs[0]+")"
            else:
                return "("+str(symbol)+" "+" ".join(rhs)+")"
            # return "("+str(symbol)+" "+self.gentree( self.random_expansion(symbol)[0])
        elif symbol == 'ROOT':
            if 'is' in rhs:
                return "("+str(symbol)+" "+" ".join(rhs[:4])+" "+ self.gentree(rhs[-2]) +" "+rhs[-1]+")"
            return "("+str(symbol) +" "+ self.gentree(rhs[0]) +" "+rhs[-1]+")"
        else:

            return "("+str(symbol) +" "+ self.gentree(rhs[0]) +" "+ self.gentree(rhs[1])+")"
        ### END YOUR CODE
