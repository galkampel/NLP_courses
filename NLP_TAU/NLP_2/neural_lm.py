...
def lm_wrapper(in_word_index, out_word_index, num_to_word_embedding, dimensions, params):
    ...
    ### YOUR CODE HERE
    batch_indices = np.random.choice(len(in_word_index), size=BATCH_SIZE, replace=False)
    data[:] = [num_to_word_embedding[in_word_index[i]] for i in batch_indices]
    labels[:] = [int_to_one_hot(out_word_index[n], output_dim) for n in batch_indices]
...
def eval_neural_lm(eval_data_path):
    ...
    for i in range(0,num_of_examples,BATCH_SIZE):
        ...
        for j in range(BATCH_SIZE):
            data = num_to_word_embedding[in_word_index[i+j]]
            label = out_word_index[j]
            p_i *= forward(data, label, params, dimensions)
        l += np.log2(p_i)
    l /= num_of_examples
