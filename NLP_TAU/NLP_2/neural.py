...
def forward_backward_prop(data, labels, params, dimensions):
    ...
    v2 = np.dot(z1,W2) + b2
    z2 = softmax(v2)
    cost = -np.sum(np.multiply(labels,np.log(z2)))
    ### END YOUR CODE

    ### YOUR CODE HERE: backward propagation
    el2 = z2 - labels
    gradb2 = np.dot(np.ones((1,N)),el2)
    gradW2 = np.dot(z1.transpose(),el2)

    el1 = np.multiply(np.dot(el2,W2.transpose()),sigmoid_grad(z1))

    gradb1 = np.dot(np.ones((1,N)),el1)
    gradW1 = np.dot(data.transpose(),el1)
    ### END YOUR CODE
