    ...
    for sentence, labels in data:
        ...
        for i,word in enumerate(sentence):
            ...
            for j in range(i-window_size,i+window_size+1):
                if j < 0:
                    window_words.extend(start)
                elif j>= 0 and j< num_words:
                    window_words.extend(sentence[j])
                else:
                    window_words.extend(end)
            windowed_data.append((window_words,labels[i]))
		### END YOUR CODE
...
class WindowModel(NERModel):
    ...
    def add_prediction_op(self):
        ...
        config = self.config
        W = tf.get_variable(name='W',shape=(config.n_window_features * config.embed_size,config.hidden_size),
                            dtype=tf.float32, initializer=tf.contrib.layers.xavier_initializer())
        b1 = tf.get_variable(name='b1',shape=(config.hidden_size),initializer=tf.zeros_initializer())
