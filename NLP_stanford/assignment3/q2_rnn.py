...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        if sentence_size < max_length:
            padding_size = (max_length - sentence_size)
            zero_padding = [zero_vector] * padding_size
            new_sentence = sentence  + zero_padding
        ...
        masking = [True if i < sentence_size else False for i in range(max_length)]
...
class RNNModel(NERModel):
    ...
    def add_prediction_op(self):
        ...
        b_2 = tf.get_variable('b_2', shape=(self.config.n_classes), dtype=tf.float32, initializer=tf.zeros_initializer())
        ...
        with tf.variable_scope("RNN"):
            for time_step in range(self.max_length):
                ...
                o_drop_t = tf.layers.dropout(o_t, dropout_rate)
    ...
    def add_loss_op(self, preds):
        ...
        """
        ### YOUR CODE HERE (~2-4 lines)
        CE_masked = tf.boolean_mask(tf.nn.sparse_softmax_cross_entropy_with_logits(labels=self.labels_placeholder,
                                                              logits=preds),self.mask_placeholder)
        loss = tf.reduce_mean(CE_masked)
        ### END YOUR CODE
        return loss
