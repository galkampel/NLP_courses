...
class GRUCell(tf.nn.rnn_cell.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope):
            ...
            U_r = tf.get_variable("U_r",shape=(self.state_size,self.state_size),dtype=tf.float32,
                                  initializer=tf.contrib.layers.xavier_initializer())
            U_z = tf.get_variable("U_z",shape=(self.state_size,self.state_size),dtype=tf.float32,
                                  initializer=tf.contrib.layers.xavier_initializer())
            U_h = tf.get_variable("U_o",shape=(self.state_size,self.state_size),dtype=tf.float32,
                                  initializer=tf.contrib.layers.xavier_initializer())
            W_r = tf.get_variable("W_r",shape=(self.input_size,self.state_size),dtype=tf.float32,
                                  initializer=tf.contrib.layers.xavier_initializer())
            W_z = tf.get_variable("W_z", shape=(self.input_size, self.state_size), dtype=tf.float32,
                                  initializer=tf.contrib.layers.xavier_initializer())
            W_h = tf.get_variable("W_o", shape=(self.input_size, self.state_size), dtype=tf.float32,
                                  initializer=tf.contrib.layers.xavier_initializer())
            ...
            b_z = tf.get_variable("b_z", shape=(self.state_size), dtype=tf.float32,
                                  initializer=tf.constant_initializer())
            b_h = tf.get_variable("b_o", shape=(self.state_size), dtype=tf.float32,
                                  initializer=tf.constant_initializer())
            ...
            o_t = tf.nn.tanh(tf.matmul(inputs,W_h)+tf.matmul(tf.multiply(r_t,state),U_h)+b_h)
