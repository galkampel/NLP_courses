...
class ParserModel(Model):
    ...
    def add_placeholders(self):
        ...
        self.labels_placeholder = tf.placeholder(tf.float32, shape=(None, Config.n_classes), name="labels")
    ...
    def add_embedding(self):
        ...
        tmp_tensor = tf.nn.embedding_lookup(embedded,self.input_placeholder)
        embeddings = tf.reshape(tmp_tensor,shape=(-1, Config.n_features*Config.embed_size))
        ### END YOUR CODE
        return embeddings
    ...
    def add_prediction_op(self):
        ...
        shape_W = (Config.n_features*Config.embed_size,Config.hidden_size)
        W = xavier_initializer(shape_W)
        shape_b1 = (Config.hidden_size,)
        b1 = tf.zeros(shape_b1)
        h = tf.nn.relu(tf.add(tf.matmul(x,W,name= 'xW'),b1,name = 'z1'),name = 'h')
        h_drop = tf.nn.dropout(h,self.dropout_placeholder)
        shape_U = (Config.hidden_size, Config.n_classes)
        U = xavier_initializer(shape_U)
        shape_b2 = (Config.n_classes,)
        b2 = tf.zeros(shape_b2)
        pred = tf.add(tf.matmul(h_drop,U,name='h_dropU'),b2,name='y_hat')
        ### END YOUR CODE
        return pred
